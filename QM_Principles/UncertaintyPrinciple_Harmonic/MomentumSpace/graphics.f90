...
!-----------------------------!
 ...
 write(1,*)'-200 0 moveto 400 0 rlineto 0 320 rlineto -400 0 rlineto'
 ...
 write(1,*) '-145 375 moveto'
 write(1,*) '(UNCERTAINTY PRINCIPLE) show'
 write(1,*) '-130 350 moveto'
 write(1,*) '([Harmonic Potential - M.S.]) show'
 write(1,*)'75 325 moveto'
 write(1,10)t/fs2au
 ...
 write(1,11) 5.0
 write(1,*)'-40 -60 moveto'
 write(1,*) '(p [2) show'
 write(1,*)'/Symbol findfont 24 scalefont setfont'
 write(1,*)'-3 -60 moveto'
 write(1,*)'(p) show'
 write(1,*)'/TimesRoman findfont 24 scalefont setfont'
 write(1,*)'15 -60 moveto'
 write(1,*) '(/a.u.]) show'
 write(1,*) 'gsave'
 write(1,*)'-250 130 moveto'
 write(1,*)'((p,t)|) 90 rotate show'
 write(1,*) 'grestore'
 write(1,*) 'gsave'
 write(1,*)'/Symbol findfont 24 scalefont setfont'
...
!-------------------------------!
 subroutine graphpsi(dx,dp,npoints,psi)
!-------------------------------!
 implicit none

 integer :: i,j,npoints
 real(8) :: x,dx,dp,psi(npoints)
 logical :: firstline

 real(8) :: p

 firstline=.true.
 write(1,*)'gsave 0 0 0 setrgbcolor 2 setlinewidth'
 do i=-npoints/2+1,npoints/2
    ...
    p=dble(i-1)*dp
!    if (abs(p)>=1.d0) then
     if (abs(psi(j)).gt.1.d-5) then
      if (firstline) then
         write(1,20)10.d0*p,10.d0*psi(j)
         write(*,*) j, p, psi(j)
         20 format(f7.2,' ',f7.2,' m')
         firstline=.false.
      else
         write(1,10)10.d0*p,10.d0*psi(j)
         write(*,*) j, p, psi(j)
